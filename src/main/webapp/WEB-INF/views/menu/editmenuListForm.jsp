		...
		<div class="container">
			...
			<div class="context-top">
				...
				<div class="coffee-description">
					<div class="name">
						<div class="name-left">
							<h3>한글 이름</h3><input type="text" name="nameKr" value="${menu.nameKr}"/>
							<h3>영어 이름</h3><input type="text" name="nameEn" value="${menu.nameEn}"/>
						</div>
			...
			<div class="context-description-bottom">
				<div class="description-bottom-left">
					...
					<div class="description-bottom-left-bottom">
						<div class="design-story">
							...
							<h4>디자인 스토리</h4>
							<textarea  name=designStory >${menu.designStory}</textarea>
						</div>
						<div class="tasting-note">
							<img src="/images/productInvestigate_img02.jpg"/>
							<h4>커피 테이스팅 노트</h4>
							<textarea  name=coffeeTastingNote>${menu.coffeeTastingNote}</textarea>
						</div>
					</div>
				</div>
				<div class="description-bottom-right">
					<div class="right-block tasting_notes">
						<h4>Tasting Notes</h4>
						<textarea name=tastingNotes >${menu.tastingNote}</textarea>
					</div>
					<div class="right-block enjoy_with">
						<h4>Enjoy With</h4>
						<textarea name=enjoyWith >${menu.enjoyWith}</textarea>
					</div>
					<div class="right-block processing_method">
						<h4>Processing Method</h4>
						<textarea name=processingMethod>${menu.processingMethod}</textarea>
					</div>
				</div>
			</div>
			...
			<div class="context-bottom" >
				...
				<a href="/coffee/deleteCoffeeBean?id=${menu.id}"><button id="cancel" class="btn btn-primary submit" type="button">취소</button></a>
			</div>
...
<script>
	...
	function valueOfFlavors(){
		const kinds = [...document.querySelector('select[name="flavor"]').children];
		for(const k of kinds){
			if(k.value === "${menu.flavor}"){
				k.classList.add('selected');
				break;
			}
		}
	}
	function valueOfFeels(){
		const kinds = [...document.querySelector('select[name="feel"]').children];
		for(const k of kinds){
			if(k.value === "${menu.feel}"){
				k.classList.add('selected');
				break;
			}
		}
	}
	function valueOfDegree(){
		const kinds = [...document.querySelector('select[name="degree"]').children];
		for(const k of kinds){
			if(k.value === "${menu.degree}"){
				k.classList.add('selected');
				break;
			}
		}
